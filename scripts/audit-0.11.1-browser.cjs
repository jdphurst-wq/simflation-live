...
(async()=>{
 ...
 const ledger=await page.evaluate(()=>window.__sim.resolutionLedger());
 const forbidden=ledger.filter(x=>String(x.route||'').includes('central-bank')||String(x.route||'').includes('stability bank'));
 await browser.close();
 ...
 for(const run of runs){if(run.halted||run.problems.length)throw new Error(`Seed ${run.seed} failed 150-year audit: ${JSON.stringify(run.problems)}`);if(run.maxBridgeAge>60)throw new Error(`Seed ${run.seed} bridge age ${run.maxBridgeAge}`);if(run.maxConsecutiveOneBank>24)throw new Error(`Seed ${run.seed} one-bank run ${run.maxConsecutiveOneBank}`)}

...
replaceFunction('updateBanks',`function updateBanks(){
    ...
    for(const bank of activeBanks().slice()){
      ...
      let equity=bankEquity(bank),capital=bankCapitalRatio(bank);const prudentialTrigger=equity<0||capital<requirement+.025;if(prudentialTrigger){const gap=Math.max(0,bankRWA(bank)*targetCapital-equity),raiseCap=Math.min(annualGDPReference()*.01,Math.max(800000*nominalScale(),bankRWA(bank)*.20)),raised=v0112RaisePrivateBankCapital(bank,Math.min(gap,raiseCap),false);if(raised>0){privateRecaps++;recordBankResolution({bankId:bank.id,bankName:bank.name,route:'private prudential recapitalisation',privateCapital:raised,governmentSupport:0,centralBankBackstop:0});equity=bankEquity(bank);capital=bankCapitalRatio(bank)}}

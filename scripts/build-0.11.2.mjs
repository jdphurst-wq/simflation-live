...
insertAfterFunction('privateCreditRoom',`
  ...
  function bankOriginationBudget(bank){
    ...
    const monthlyGrowth=Math.max(18000*nominalScale(),book*.018+equity*.10),fundingRoom=Math.max(15000*nominalScale(),deposits*.045+Math.max(0,bank.reserves||0)*.20);
...
replaceFunction('createCredit',`function createCredit(borrower,requested,type='business'){
    ...
    const gdp=Math.max(1,annualGDPReference()),revenue=Math.max(12000*nominalScale(),borrower.expectedRevenue||0),assetBase=type==='developer'?Math.max(0,propertyAssetValue(borrower)) : Math.max(0,borrower.capitalValue||0),borrowerLimit=type==='developer'?Math.min(gdp*.10,assetBase*.70+gdp*.01):Math.min(gdp*.035,revenue*4+assetBase*.45),concentrationLimit=Math.max(25000*nominalScale(),bankEquity(bank)*(type==='developer'?.12:.08)),room=Math.max(0,Math.min(borrowerLimit,concentrationLimit)-(borrower.debt||0));
...
replaceFunction('updateBanks',`function updateBanks(){
    ...
    for(const bank of activeBanks().slice()){
      ...
      const severe=equity<-Math.max(60000*nominalScale(),deposits*.08);if(severe||bank.insolvencyMonths>=3){resolveBank(bank);continue}bank.quietMonths=deposits<12000*nominalScale()&&bankFirmLoans(bank.id)+bankMortgages(bank.id)<15000*nominalScale()?bank.quietMonths+1:0
    }

...
insertAfterFunction('privateCreditRoom',`
  ...
  function bankOriginationBudget(bank){
    ...
    const monthlyGrowth=Math.max(25000*nominalScale(),book*.04+equity*.22),fundingRoom=Math.max(22000*nominalScale(),deposits*.08+Math.max(0,bank.reserves||0)*.35);
  ...
  function v0112ResolutionCapital(bank,requested,route){
    ...
    const cap=Math.min(annualGDPReference()*.0035,Math.max(60000*nominalScale(),bankRWA(bank)*.055)),amount=Math.min(Math.max(0,requested),cap),paid=governmentPaysBank(bank,amount,'bankSupportSpending');
...
replaceFunction('createCredit',`function createCredit(borrower,requested,type='business'){
    ...
    const gdp=Math.max(1,annualGDPReference()),revenue=Math.max(12000*nominalScale(),borrower.expectedRevenue||0),assetBase=type==='developer'?Math.max(0,propertyAssetValue(borrower)) : Math.max(0,borrower.capitalValue||0),borrowerLimit=type==='developer'?Math.min(gdp*.14,assetBase*.72+gdp*.012):Math.min(gdp*.075,revenue*10+assetBase*.55),concentrationLimit=Math.max(35000*nominalScale(),bankEquity(bank)*(type==='developer'?.25:.20)),room=Math.max(0,Math.min(borrowerLimit,concentrationLimit)-(borrower.debt||0));
...
replaceFunction('suitableBankAcquirer',`function suitableBankAcquirer(excludeId,name='Successor Bank'){
    const requirement=Math.max(.04,Number($('capitalRequirement').value)/100);let acquirer=activeBanks().filter(b=>b.id!==excludeId&&bankEquity(b)>0&&bankCapitalRatio(b)>=requirement).sort((a,b)=>bankCapitalRatio(b)-bankCapitalRatio(a))[0];if(acquirer)return acquirer;
...
replaceFunction('completeBankTransfer',`function completeBankTransfer(failed,successor,customers,target,details={}){
    ...
    const finalLosses=stabiliseResolvedBank(successor,customers,target),governmentSupport=(details.support||0)+(finalLosses.governmentSupport||0);recordBankResolution({failedBank:failed.name,successorBank:successor.name,route:details.route||'resolution',openingEquity:details.openingEquity,openingDeposits:details.openingDeposits,openingAssets:details.openingAssets,uninsuredDepositLoss:details.bailIn||0,governmentSupport,priorBailoutSupport:details.priorBailoutSupport||0,recognisedImpairedAssets:Math.max(0,successor.resolutionProvision||0),...finalLosses,governmentSupport});return finalLosses
  }`);
replaceFunction('resolveBank',`function resolveBank(bank){
    ...
    if(selectedPolicy==='bailout'&&!bridgeEpisode&&(bank.lastBailoutIndex===null||bank.lastBailoutIndex===undefined||monthIndex-bank.lastBailoutIndex>=120)){
      const target=Math.max(.14,requirement+.04),needed=Math.max(0,bankRWA(bank)*target-bankEquity(bank)),cap=Math.min(annualGDPReference()*.005,bankRWA(bank)*.15),support=governmentPaysBank(bank,Math.min(needed,cap),'bankSupportSpending');if(support>0)bank.publicResolutionEquity=(bank.publicResolutionEquity||0)+support;
      if(bankEquity(bank)>=0&&bankCapitalRatio(bank)>=requirement){bank.lastBailoutIndex=monthIndex;bank.supportPlanMonths=120;bank.resolutionCooldown=36;bank.insolvencyMonths=0;bank.underCapitalMonths=0;recordBankResolution({failedBank:bank.name,successorBank:bank.name,route:'bailout',openingEquity,openingDeposits,openingAssets,governmentSupport:support,centralBankBackstop:0,finalEquity:bankEquity(bank),finalCapitalRatio:bankCapitalRatio(bank)});resolvingBankId=null;return}
    }

...
insertAfterFunction('privateCreditRoom',`
  ...
  function bankOriginationBudget(bank){
    if(!bank?.active)return 0;
    const book=Math.max(0,bankFirmLoans(bank.id)+bankMortgages(bank.id)),equity=Math.max(0,bankEquity(bank)),deposits=Math.max(0,bankDeposits(bank.id)),ramp=bank.entryRampMonths>0?clamp((25-bank.entryRampMonths)/24,.25,1):1;
...
replaceFunction('bankCreditCapacity',`function bankCreditCapacity(bank,riskWeight=1,recipientBankId=null){
    ...
    const requirement=Math.max(.04,Number($('capitalRequirement').value)/100),equity=bankEquity(bank),rwa=bankRWA(bank),book=bankFirmLoans(bank.id)+bankMortgages(bank.id),capitalRoom=Math.max(0,(equity/requirement-rwa)/Math.max(.05,riskWeight)),leverageRoom=Math.max(0,equity*25-bankAssets(bank)),growthRoom=Math.max(0,bankOriginationBudget(bank)-Math.max(0,bank.monthOriginations||0)),concentrationRoom=Math.max(0,annualGDPReference()*.52-book);
    let capacity=Math.min(capitalRoom,leverageRoom,growthRoom,concentrationRoom,privateCreditRoom());if(recipientBankId!==null&&recipientBankId!==bank.id)capacity=Math.min(capacity,bankSettlementCapacity(bank));return Math.max(0,capacity)
  }`);
...
replaceFunction('capitaliseNewBank',`function capitaliseNewBank(bank,target=Math.max(300000*nominalScale(),annualGDPReference()*.0065)){
    const raised=v0112RaisePrivateBankCapital(bank,target,true),viable=raised>=target*.88&&raised>=Math.max(240000*nominalScale(),annualGDPReference()*.005);
...
replaceFunction('stabiliseResolvedBank',`function stabiliseResolvedBank(bank,customers,target){
    const requirement=Math.max(.04,Number($('capitalRequirement').value)/100),requiredEquity=Math.max(Math.max(0,target||0),bankRWA(bank)*Math.max(.12,requirement+.02));let gap=Math.max(0,requiredEquity-bankEquity(bank)),governmentDepositLoss=0,insuredDepositLoss=0,additionalBailIn=0,remaining=gap,governmentSupport=0;if(gap>0){if($('bankPolicy').value==='noSupport'){additionalBailIn=applyUninsuredDepositBailIn(customers,gap);remaining=Math.max(0,requiredEquity-bankEquity(bank));if(remaining>0){const losses=applyResidualResolutionLosses(customers,remaining);governmentDepositLoss=losses.governmentDepositLoss;insuredDepositLoss=losses.insuredDepositLoss;remaining=losses.remaining}}else{governmentSupport=v0112ResolutionCapital(bank,gap,'deposit-guarantee resolution support');remaining=Math.max(0,requiredEquity-bankEquity(bank));if(remaining>0){const losses=applyResidualResolutionLosses(customers,remaining);governmentDepositLoss=losses.governmentDepositLoss;insuredDepositLoss=losses.insuredDepositLoss;remaining=losses.remaining}}}
    return{governmentDepositLoss,insuredDepositLoss,additionalBailIn,remaining,centralBankWriteDown:0,centralBankBackstop:0,governmentSupport,finalEquity:bankEquity(bank),finalCapitalRatio:bankCapitalRatio(bank)}
  }`);
replaceFunction('suitableBankAcquirer',`function suitableBankAcquirer(excludeId,name='Successor Bank'){
    const requirement=Math.max(.04,Number($('capitalRequirement').value)/100),failedBook=bankFirmLoans(excludeId)+bankMortgages(excludeId),concentrationCap=annualGDPReference()*.55;let acquirer=activeBanks().filter(b=>b.id!==excludeId&&bankEquity(b)>0&&bankCapitalRatio(b)>=requirement&&bankFirmLoans(b.id)+bankMortgages(b.id)+failedBook<=concentrationCap).sort((a,b)=>(bankEquity(b)+Math.max(0,b.reserves||0))-(bankEquity(a)+Math.max(0,a.reserves||0)))[0];if(acquirer)return acquirer;
...
replaceFunction('resolveBank',`function resolveBank(bank){
    ...
    if(!bridgeEpisode)failures++;const insolvency=Math.max(0,-bankEquity(bank)),bailIn=applyUninsuredDepositBailIn(customers,insolvency);let residual=Math.max(0,insolvency-bailIn),preSupport=0;
...
replaceFunction('updateBanks',`function updateBanks(){
    ...
    for(const bank of activeBanks().slice()){
      ...
      let equity=bankEquity(bank),capital=bankCapitalRatio(bank);const prudentialTarget=Math.max(.16,requirement+.06),prudentialTrigger=equity<0||capital<targetCapital;if(prudentialTrigger){const gap=Math.max(0,bankRWA(bank)*prudentialTarget-equity),raiseCap=Math.min(annualGDPReference()*.015,Math.max(1000000*nominalScale(),bankRWA(bank)*.30)),raised=v0112RaisePrivateBankCapital(bank,Math.min(gap,raiseCap),false);if(raised>0){privateRecaps++;recordBankResolution({bankId:bank.id,bankName:bank.name,route:'private prudential recapitalisation',privateCapital:raised,governmentSupport:0,centralBankBackstop:0});equity=bankEquity(bank);capital=bankCapitalRatio(bank)}}
      const liquidity=bankLiquidityStress(bank);bank.lossMonths=bank.monthInterest+reserveInterest<bank.monthCosts?bank.lossMonths+1:Math.max(0,bank.lossMonths-1);bank.liquidityDistressMonths=liquidity>.80?bank.liquidityDistressMonths+1:Math.max(0,bank.liquidityDistressMonths-1);bank.underCapitalMonths=capital<requirement?bank.underCapitalMonths+1:Math.max(0,bank.underCapitalMonths-1);bank.insolvencyMonths=equity<0?bank.insolvencyMonths+1:Math.max(0,bank.insolvencyMonths-1);
    ...
    const count=activeBanks().length,candidate=activeBanks().filter(b=>count>minimum&&!b.name.startsWith('Bridge Bank')&&b.age>36&&(b.quietMonths>12||b.underCapitalMonths>12)).sort((a,b)=>bankCapitalRatio(a)-bankCapitalRatio(b))[0];if(candidate){const candidateBook=bankFirmLoans(candidate.id)+bankMortgages(candidate.id),acquirer=activeBanks().filter(b=>b.id!==candidate.id&&bankEquity(b)>0&&bankCapitalRatio(b)>=requirement&&bankFirmLoans(b.id)+bankMortgages(b.id)+candidateBook<=annualGDPReference()*.55).sort((a,b)=>bankCapitalRatio(b)-bankCapitalRatio(a))[0];if(acquirer){const customers=bankCustomers(candidate.id);completeBankTransfer(candidate,acquirer,customers,0,{route:'voluntary merger',openingEquity:bankEquity(candidate),openingDeposits:bankDeposits(candidate.id),openingAssets:bankAssets(candidate),bailIn:0,support:0});lastBankResolutionText=candidate.name+' merged into '+acquirer.name}}
